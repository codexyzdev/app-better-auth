...
// --- Extracted components (not inline) per rerender-no-inline-components rule ---
...
function SidebarUser({ name, email, initials, onSignOut }: SidebarUserProps) {
    return (
...
function Sidebar({ pathname, name, email, initials, onSignOut, mobile = false }: SidebarProps) {
    return (
        <div className={`flex flex-col h-full ${mobile ? "p-4" : "p-5"}`}>
            ...
            <SidebarUser name={name} email={email} initials={initials} onSignOut={onSignOut} />
        </div>
    );
}
